...
//cycle program time values
long startTime = 0;                                       // start time of the overall program
long checkpoint1 = 3000;                                  // time delta of the first checkpoint
long checkpoint2 = 6000;                                  // time delta of the second checkpoint
long checkpoint3 = 9000;                                  // time delta of the third checkpoint
bool firstRun = true;                                     // flag for finding out if the first cycle is currently active
...
float target4 = 75;                                       // target value for servo 4
...
void runProgram() {
  if(firstRun) {                                          // if the first cycle is currently active
    firstRun = false;                                     // reset the first run flag to avoid overwriting the start time
    startTime = millis();                                 // set the start time of the cycle program
  }
  long currentDelay = millis() - startTime;               // calculate current delay from start
  if(currentDelay >= checkpoint1 && currentDelay < checkpoint2) { // if the first checkpoint is reached
    setNewTarget(1,25);                                  // set new target position 1
    setNewTarget(2,100);                                  // set new target position 2
    setNewTarget(4,150);                                  // set new target position 4
    setNewTarget(5,100);                                  // set new target position 5
    setNewTarget(8,100);                                  // set new target position 8
    setNewTarget(11,90);                                  // set new target position 11
    setNewTarget(13,160);                                  // set new target position 13
    setNewTarget(14,80);                                  // set new target position 14
    setNewTarget(16,25);                                 // set new target position 16
    setNewTarget(17,90);                                  // set new target position 17
  } else if(currentDelay >= checkpoint2 && currentDelay < checkpoint3) { // if the second checkpoint is reached
    setNewTarget(3,95);                                  // set new target position 3
    setNewTarget(6,90);                                  // set new target position 6
    setNewTarget(9,90);                                  // set new target position 9
    setNewTarget(12,95);                                 // set new target position 12
    setNewTarget(15,80);                                 // set new target position 15
    setNewTarget(18,90);                                 // set new target position 18
  } else if(currentDelay >= checkpoint3) {                // if the third checkpoint is reached
    setNewTarget(2,25);                                   // set new target position 2
    setNewTarget(5,26);                                   // set new target position 5
    setNewTarget(8,20);                                   // set new target position 8
    setNewTarget(11,20);                                  // set new target position 11
    setNewTarget(14,10);                                  // set new target position 14
    setNewTarget(17,20);                                  // set new target position 17
    ;
  }
  ...
  setServoByNumber(17,servoModel(old17, oldTarget17, target17)); // set new servo value 17
  setServoByNumber(18,servoModel(old18, oldTarget18, target18)); // set new servo value 18
}
...
void loop() {
  if((millis()-previousTime) >= (cycleTime*1000)) {       // if enough time has passed between individual loops
    previousTime = millis();                              // save current timestamp for next cycle
